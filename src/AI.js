// AI Factory to return AI of some difficulty
class AIFactory {
  constructor() {}
...
class AI {
  ...
  selectTarget() {
    return null;
  } // returns list [i,j] of coordinates to fire at
...
class HuntTargetAI extends AI {
  ...
  updateHit(i, j, hit = true, ship = null) {
    if (hit) {
      ...
      if (i < N - 1 && this.board[i + 1][j] == 0)
        this.targetStack.push([i + 1, j]);
      if (j > 0 && this.board[i][j - 1] == 0) this.targetStack.push([i, j - 1]);
      if (j < N - 1 && this.board[i][j + 1] == 0)
        this.targetStack.push([i, j + 1]);
    }
  }
}
...
class ProbabilityAI extends AI {
  ...
  checkHorizontal(statusBoard, densityBoard, i0, j0, length) {
    ...
    for (let j = j0; j < j0 + length; j++) {
      if (statusBoard[i0][j] === this.UNVISITED)
        densityBoard[i0][j] += multiplier;
    }
  }
  ...
  checkVertical(statusBoard, densityBoard, i0, j0, length) {
    ...
    // update density board
    for (let i = i0; i < i0 + length; i++) {
      if (statusBoard[i][j0] === this.UNVISITED)
        densityBoard[i][j0] += multiplier;
    }
  }
